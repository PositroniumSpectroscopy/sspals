...
from sspals.core import sub_offset, chmx, cfd, validate, sspals_1D
...
def test_chmx():
    ...
    assert (chmx(high, low, n_bsub=0, invert=False) == \
...
def test_cfd():
    ...
    assert cfd(x_vals, 1, cfd_offset=2, cfd_threshold=1,
                      cfd_scale=0.6) == 2.0
def test_validate():
    """ check validation -> ignore empty rows.
    """
    arr = np.array([[0.0, 2.0], [-2.0, -1.5], [0.0, 10.0]])
    assert (validate(arr, min_range=0.5) == \
