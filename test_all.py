...
def test_sub():
    ...
    """
    assert (sspals.sub_offset(np.array([np.arange(1.0, 10.0)]), 3)[0][0] == \
...
def test_chmx():
    ...
    assert (sspals.chmx(high, low, n_bsub=0, invert=False) == \
            [[0.0, 0.6, 0.5, 3.0, 4.0, 5.0]]).all()

def test_cfd():
    """ constant fraction discriminator test on linear ramp.
    """
    x_vals = np.arange(1, 10)
    assert sspals.cfd(x_vals, 1, cfd_offset=2, cfd_threshold=1,
                      cfd_scale=0.6) == 2.0
def test_validate():
    """ check validation -> ignore empty rows.
    """
    arr = np.array([[0.0, 2.0], [-2.0, -1.5], [0.0, 10.0]])
    assert (sspals.validate(arr, min_range=0.5) == \
            np.array([[0.0, 2.0], [0.0, 10.0]])).all()

def test_sspals():
    """ simulate sspals and measure the delayed fraction.
    """
    x_vals = np.arange(-100, 600, 1) * 1e-9
    y_vals = sspals.sim(x_vals, amp=1.0, sigma=2e-09, eff=0.4,
                        tau_Ps=1.420461e-07, tau_d=1e-08)
    assert round(sspals.sspals_1D(y_vals, 1e-9)[0]['DF'], 6) == 0.354113
